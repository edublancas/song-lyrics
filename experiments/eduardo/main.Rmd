...
```{r}
tags <- table(terms$mbtag)
wordcloud(names(tags), tags)
```
...
```{r}
# filter by tag and then sample artists
```

Cannot use the same approach since one tag corresponds to many artists.
