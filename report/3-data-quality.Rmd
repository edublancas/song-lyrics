...
Besides the top 5,000 must frequent words in the lyrics of the Million Songs dataset, our dataset also include an additional 16 variables describing the songs and artists. These variables are used in this section to explore missing values among them.  The folliwng barchart present the missing vales rate per variable. We can see that 10 of this 16 variables are missing value free and more than half of the songs on our dataset have missing artist location information (location, latitude and longitude). Is also important to mentioned that a little over one quarter of the song in our dataset have missing genre.  
...
The following plot shows that the prevalent missing pattern corresponds to the cobination of variables location, latitude and longitud. This pattern is followed by the cobination of the same variables with release year and then by the release year variable by itself.

```{r missingPattern}
extracat::visna(metadata,sort = 'b')
```

The following Heapmap show the distribution of missing values by language. 
...
Per song counts must be carefully considered, because they only count words which are in the most common 5000 words. Nevertheless, we can see that the mode is around 75 words, and there is a wide range of word counts. When considering songs in this project, it is important to note that songs with different word counts will behave differently.
