...
Every song is represented as a vector $C \in \mathbb{R}^{|W|}$, where $W$ are the words in our dataset. Every element $c_{word}$ in $C$ has a word associated with it and it represents the number of times that word is mentioned in the song. To convert this to a dense vector, we first normalized it:

$$C_{normalized} = \frac{C}{\sum_{i=1}^{i=w}{C_i}}$$


Then, using the $w \in \mathbb{R}^{50}$ dense vectors in GloVe, we computed a single vector for every song as follows:


First, initialize a zero vector $V \in \mathbb{R}^{50}$

For every non-zero value $c_{normalized, word}$ in $c_{normalized}$:
    1. Lookup the corresponding embedding $w_{word}$
    2. Compute $c_{normalized, word} \times w_{word}$
    3. Add  $c_{normalized, word} \times w_{word}$ to $V$

$V$ is the dense vector representation for the song.
