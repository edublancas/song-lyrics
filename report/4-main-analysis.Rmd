...
Then, using the $w \in \mathbb{R}^{50}$ dense vectors in GloVe, we built a matrix where the ith row corresponds to the embedding for the i-th word in $|W|$, then we compute the dense vector for every song as follows:
...
2. `bag_of_words.feather` - Contains the counts top 1,000 words with the stop words removed
...
Here we see that C??line Dion looks brighter, meaning that her lyrics are different from the rest of the artists.
