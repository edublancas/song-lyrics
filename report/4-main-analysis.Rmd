...
The require R packages for this analysis are the following:
...
Lets start our analysis by looking at the top 200 most frequent words in our dataset using a word cloud.
...
We can see from the previous visualization that the most used word in this dataset is the word love, followed by words like time, feel, baby, yeah, heart, etc. By adding the bing sentiment lexicon to the previous word cloud, we can see the most frequent positive and negative words. Love is the most frequent positive word used as expected, follow by words like heaven, promise, beautiful, free, smile, sweet, ext. On the negative side we can see words like die, fall, pain, wrong, hard, cry, lies, etc.
...
songsDataEng %>% filter(genre_ %in% names(sort(table(songsDataClean$genre_),decreasing = TRUE)[1:15])) %>%
  ...
  ggplot(aes(y=meanSentiment,x=artist_name_)) + geom_point(size=2) + 
...
```{r, fig.cap="Distance among average songs accross top genres (Spanish)"}
...
```{r, fig.cap="Distance among average songs accross top genres (English)"}
...
Here we see that C??line Dion looks brigther, meaning that her lyrics are different from the rest of the artists.
...
Using these topic weights, each song can be scored to show how prevelent each topic is. In order to account for songs with many more words than others, these topic scores are divided by the number of words in each song to ensure correct equivilent scaling. However, just because a song has a higher religion score than death score does not mean it is more about religion, therefore, we must scale each topic to lie within common bounds. To do this, each song's topic score is ranked according to its sorted order, and then the ranking is scaled between 0 and 1.

Now we can begin to answer some fundemental questions with the topic data:
...
* In general, topics have become more consistant each year (based on preprocessing, each topic's mean lies at 0.5, and all topics aproach 0.5 over time).
...
In order to investigate topics around the world, each artist's topic scores are averaged. Only artists maintaining an average score of over 0.75 for a particular topic are considered to consistantly discuss that topic.  By using this cuttoff, have we defined approximately 11% of artists to speak heavily about death, ~14% of artists to conss speak heavily about love, and 11% of artists to speak heavily about religion. 
...
ggplot(bardata, aes(x = reorder(topic, count), y=count))+
  geom_col() + 
  ggtitle('Artists Count Consistantly Referring to a Particular Topic')+
  xlab('Topic')+
  ylab('Count')
