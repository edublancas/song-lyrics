//Determine if viewed on mobile device
...
//Update the search box with artist names
...
function searchArtist(artist) {
	...
	if (selectedArtist != "") {
	 	d3.csv(fileName, function(error, data) {
			let index = data.findIndex(data => data.artist == selectedArtist);
			artist_name.text(data[index].artist);
			artist_song.text(data[index].songs);

			var genres = JSON.parse(JSON.stringify(eval("(" + data[index].genres + ")")));


			if (genres.length >= 3) {
				artist_genres.text(genres[0] + "\r\n" + genres[1] + "\n" + genres[2]);
			}
			...
			artist_years.text(data[index].mean_sentiment)
			
			var similar = JSON.parse(JSON.stringify(eval("(" + data[index].similar + ")")));
			

			if (similar.length >= 3) {
				artist_score.text(similar[0] + "\r\n" + similar[1] + "\n" + similar[2]);
			}
			...
			var json = JSON.parse(JSON.stringify(eval("(" + data[index].words + ")")));
			...
			var bars = barContainer.selectAll(".bar")
										.data(json.sort(function(a, b) {return d3.descending(a.count, b.count)}) , function(d) { return d.count; });
			...
			var topic = JSON.parse(JSON.stringify(eval("(" + data[index].topic + ")")));
