...
top_per_topic_words = corpus_topics_top_words(
    mxm_25_top1000, bag_of_words_top_1000.iloc[:, 16:].columns.values, 10)
...
song_topic_weights_df = pd.DataFrame(
    data=song_topic_weights, columns=initial_topic_names)
...
original_top_1000 = pd.read_feather(
    '../../data/transform/bag_of_words_top_1000.feather')
...
# scale by word count
bag_of_words_top_1000_topics['sum'] = original_top_1000.iloc[:, 16:].sum(
    axis=1)
bag_of_words_top_1000_topics['love'] = bag_of_words_top_1000_topics['love'] / \
    bag_of_words_top_1000_topics['sum']
bag_of_words_top_1000_topics['religion'] = bag_of_words_top_1000_topics['religion'] / \
    bag_of_words_top_1000_topics['sum']
bag_of_words_top_1000_topics['death'] = bag_of_words_top_1000_topics['death'] / \
    bag_of_words_top_1000_topics['sum']
bag_of_words_top_1000_topics.drop('sum', axis=1, inplace=True)


# rank and scale between 0 and 1
rank_order = np.array(range(len(bag_of_words_top_1000_topics))
                      )/(len(bag_of_words_top_1000_topics)-1)

bag_of_words_top_1000_topics = bag_of_words_top_1000_topics.sort_values(
    by='religion')
bag_of_words_top_1000_topics = bag_of_words_top_1000_topics.reset_index(
    drop=True)
...
bag_with_year = bag_of_words_top_1000_topics[bag_of_words_top_1000_topics['release_year_'].notnull(
)].reset_index()

weights_yearly_clean = {}
weights_yearly_clean['song_count'] = [0] * \
    len(bag_with_year['release_year_'].unique())
weights_yearly_clean['love'] = [0] * \
    len(bag_with_year['release_year_'].unique())
weights_yearly_clean['death'] = [0] * \
    len(bag_with_year['release_year_'].unique())
weights_yearly_clean['religion'] = [0] * \
    len(bag_with_year['release_year_'].unique())
weights_yearly_clean['year'] = [0] * \
    len(bag_with_year['release_year_'].unique())
...
topics_per_year = pd.DataFrame(weights_yearly_clean).sort_values(
...
weights_artist_clean['love'] = [0] * \
    len(bag_of_words_top_1000_topics.artist_id_.unique())
weights_artist_clean['death'] = [0] * \
    len(bag_of_words_top_1000_topics.artist_id_.unique())
weights_artist_clean['religion'] = [0] * \
    len(bag_of_words_top_1000_topics.artist_id_.unique())
weights_artist_clean['artist_id'] = [0] * \
    len(bag_of_words_top_1000_topics.artist_id_.unique())
weights_artist_clean['artist_name'] = [0] * \
    len(bag_of_words_top_1000_topics.artist_id_.unique())
weights_artist_clean['longitude'] = [0] * \
    len(bag_of_words_top_1000_topics.artist_id_.unique())
weights_artist_clean['latitude'] = [0] * \
    len(bag_of_words_top_1000_topics.artist_id_.unique())
weights_artist_clean['song_count'] = [0] * \
    len(bag_of_words_top_1000_topics.artist_id_.unique())
...
topics_per_artist = pd.DataFrame(weights_artist_clean).sort_values(
    by='artist_name').reset_index(drop=True)

topics_per_artist.to_feather(
